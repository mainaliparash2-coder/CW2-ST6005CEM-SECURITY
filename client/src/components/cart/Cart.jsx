...
//   // FIXED: Calculate order amount with fallback to price
//   let orderAmount = 0;
//   for (let i = 0; i < cartArr.length; i++) {
//     if (cartArr[i].cartItem) {
//       let itemPrice = 0;
...
//       // Try accValue first
//       if (cartArr[i].cartItem.accValue) {
//         itemPrice = cartArr[i].cartItem.accValue;
//       }
//       // Fallback to price field (remove commas and convert to number)
//       else if (cartArr[i].cartItem.price) {
//         itemPrice = parseFloat(cartArr[i].cartItem.price.replace(/,/g, ''));
//       }

//       orderAmount += cartArr[i].qty * itemPrice;
//     }
//   }
...
//         const { key } = await axios.get("http://localhost:5000/api/get-razorpay-key");
...
//   if (cartArr.length > 0 && cartArr[cartArr.length-1]) {
...
//     let amount = Math.round(orderAmount).toString();
...
const Cart = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/getAuthUser", { withCredentials: true })
      .then(function (res) {
        setUserData(res.data);
        setCartArr(res.data.cart || []);
        setIsLoading(false);
  ...
  // Simple COD checkout - NO Razorpay
  async function placeOrderCOD() {
    try {
      setIsLoading(true);

      const res = await axios.post(
        "http://localhost:5000/api/place-order-cod",
        {},
        { withCredentials: true }
      );
