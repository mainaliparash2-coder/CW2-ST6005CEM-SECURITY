...
// Clear login restrictions (Superadmin recommended)
router.post(
  "/unblock-login",
  authenticateAdmin,
  requireSuperAdmin,
  async (req, res) => {
    try {
      ...
      // Basic safety: require at least one identifier
      if (!email && !ip) {
        return res.status(400).json({
          status: false,
          message: "Provide email or ip to unblock",
        });
      }
      ...
      res.json({
        status: true,
        message: "Login restrictions cleared",
      });
    } catch (error) {
      res.status(500).json({
        status: false,
        message: "Error clearing login restrictions",
        error: error.message,
      });
    }
  },
...
// View currently blocked logins (Superadmin only)
router.get(
  "/blocked-logins",
  authenticateAdmin,
  requireSuperAdmin,
  async (req, res) => {
    try {
      const blocked = await LoginAttempt.find({
      ...
      res.status(500).json({
        status: false,
        message: "Error fetching blocked logins",
        error: error.message,
      });
    }
  },
